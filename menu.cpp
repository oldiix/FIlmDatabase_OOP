...
void menu::loginAdmin() {
    std::cout << "Admin login: ";  std::string name; std::cin >> name;
    std::cout << "Password: ";     std::string pwd;  std::cin >> pwd;
...
void menu::displayAdminMenu() {
    while (true) {
        ...
        else if (c == 5) dynamic_cast<admin*>(currentUser.get())
                                ->viewUserActivities(users);
...
void menu::showFilmDetails() const {
    ...
    searchTitle.erase(searchTitle.find_last_not_of(" \t\n\r\f\v") + 1);
    ...
    for (const auto& filmPtr : films) {
        ...
        try {
            ...
            filmTitle.erase(0, filmTitle.find_first_not_of(" \t\n\r\f\v"));
            filmTitle.erase(filmTitle.find_last_not_of(" \t\n\r\f\v") + 1);
            ...
            if (filmTitle == searchTitle) {
                ...
                try {
                    const Director& director = filmPtr->getDirector();
                    std::cout << "\nDirector: " << director.getName()
                              << "\nAge: " << director.getAge()
                              << "\nNationality: " << director.getNationality()
                              << "\nFilms directed: " << director.getFilmsDirected();
                } catch (const std::exception& e) {
                    std::cerr << "\nError getting director info: " << e.what();
                }
                std::cout << "\n===================\n";
                break;
            }
    ...
    if (!found) {
        ...
        for (const auto& filmPtr : films) {
            if (filmPtr) std::cout << "- " << filmPtr->getTitle() << "\n";
        }
    }
}
...
void menu::addNewFilm() {
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    std::string t, g, dn, nt; int y, a, fd;
